...
function SavedMovies() {
  ...
  return (
    <>
      <h2 className='relative text-white font-bold md:text-xl p-4'>Favorite Movies</h2>
      <div className=' flex items-center group'>
        ...
        <div id={'slider'} className='w-full left-0 h-full overflow-x-scroll whitespace-nowrap scroll-smooth scrollbar-hide relative'>
          {movies?.map((item, id) => (
            <div key={id} className='w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280px] inline-block cursor-pointer relative p-2' >
              ...
              <div className='absolute top-0 left-0 w-full h-full hover:bg-black/80 opacity-0 text-white hover:opacity-100'>
                ...
                <p onClick={()=>deleteShow(item.id)} className='absolute text-red-700 top-4 right-4'><AiOutlineClose/></p>
              </div>
            </div>
          ))}
        </div>
